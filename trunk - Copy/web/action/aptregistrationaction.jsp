...
<% try {
        ...
        if ("patient".equals(request.getParameter("action"))) {
            ...
            if (lName.equalsIgnoreCase("") || fName.equalsIgnoreCase("")) {
                session.setAttribute("lasterror", "Name Required!");
                session.setAttribute("class", "alert-error");
                response.sendRedirect("../records.jsp");
                return;
            }
            ...
            if (year.equalsIgnoreCase("D") || month.equalsIgnoreCase("M") || day.equalsIgnoreCase("Y")) {
                session.setAttribute("lasterror", "Birthdate Required");
                session.setAttribute("class", "alert-error");
                response.sendRedirect("../records.jsp");
                return;
            }
            if (!contact.equals("N/A") && contact.length() > 14) {
                session.setAttribute("lasterror", "Phone Number Should be less than 14 Digits");
                session.setAttribute("class", "alert-error");
                response.sendRedirect("../records.jsp");
                return;
            }
            if (!contact.equals("N/A") && contact.length() < 10) {
                session.setAttribute("lasterror", "Phone Number length be at least 10 Digits");
                session.setAttribute("class", "alert-error");
                response.sendRedirect("../records.jsp");
                return;
            }
            if (!emergencyContact.equals("N/A") && emergencyContact.length() > 14) {
                session.setAttribute("lasterror", "Emergency Phone Number Should be less than 14 Digits");
                session.setAttribute("class", "alert-error");
                response.sendRedirect("../records.jsp");
                return;
            }
            if (!emergencyContact.equals("N/A") && emergencyContact.length() < 10) {
                session.setAttribute("lasterror", "Emergency Phone Number length be at least 10 Digits");
                session.setAttribute("class", "alert-error");
                response.sendRedirect("../records.jsp");
                return;
            }
            ...
            if (sponsorType.trim().equals("NHIS") && nhismembershipid.equalsIgnoreCase("")) {
                session.setAttribute("lasterror", "NHIS Membership Number Required!");
                session.setAttribute("class", "alert-error");
                response.sendRedirect("../records.jsp");
                return;
            }
            if (sponsorType.trim().equals("Cooperate") && coperateid.equalsIgnoreCase("N/A")) {
                session.setAttribute("class", "alert-error");
                session.setAttribute("lasterror", "Corperate Membership Number Required!");
                response.sendRedirect("../records.jsp");
                return;
            }

            if (sponsorType.trim().equals("Private") && membershipID.equalsIgnoreCase("")) {
                session.setAttribute("lasterror", "Private Insurance Membership Number Required!");
                session.setAttribute("class", "alert-error");
                response.sendRedirect("../records.jsp");
                return;
            }
